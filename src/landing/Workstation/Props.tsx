...
/** Mechanical keyboard — procedural body + key grid with breathing
 *  accent-light underglow. */
export const Keyboard: React.FC<KeyboardProps> = ({ reducedMotion }) => {
  ...
  for (let row = 0; row < 4; row++)
    for (let col = 0; col < 11; col++) keys.push([col, row]);
  return (
...
/** Ceramic mug with procedural label and rising steam particles. */
...
/** Server tower with rubber-duck konami easter-egg. Procedural body + fan +
 *  LED stack. The konami code rotates the lid open and pops a rubber duck
 *  out of the top. */
