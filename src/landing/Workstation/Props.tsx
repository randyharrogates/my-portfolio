  ...
  return (
    <group position={[0, 0.062, 0.46]}>
      ...
      <mesh castShadow receiveShadow>
        ...
        <meshStandardMaterial color="#13110f" roughness={0.6} metalness={0.3} />
      </mesh>
...
export const Trackpad: React.FC = () => (
  <mesh position={[0.62, 0.07, 0.5]} castShadow>
    ...
    <meshStandardMaterial color="#23201d" roughness={0.4} metalness={0.3} />
  </mesh>
);
  ...
  return (
    <group position={[-1.18, 0.16, 0.42]}>
      <mesh castShadow>
        ...
        <meshStandardMaterial color="#c8bfb5" roughness={0.6} />
      </mesh>
      {/* Coffee surface */}
  ...
  return (
    <group position={[-2.0, -0.46, -0.4]}>
      <mesh castShadow receiveShadow>
        ...
        <meshStandardMaterial color="#161412" roughness={0.7} metalness={0.2} />
      </mesh>
      {/* Brushed-metal front panel */}
      <mesh position={[0.215, 0, 0]}>
        <boxGeometry args={[0.005, 0.92, 0.48]} />
        <meshStandardMaterial color="#2a2825" roughness={0.4} metalness={0.85} />
      </mesh>
      {/* LEDs */}
      ...
      <group ref={lidRef} position={[0, 0.475, 0]}>
        <mesh position={[0, 0.005, 0]}>
          ...
          <meshStandardMaterial color="#1d1a17" roughness={0.6} metalness={0.4} />
        </mesh>
      </group>
...
export const ServerRackPanel: React.FC = () => (
  <mesh position={[2.18, 0.95, -0.2]} rotation={[0, -Math.PI / 2.4, 0]}>
    ...
    <meshStandardMaterial color="#0e0c0b" roughness={0.85} metalness={0.2} />
  </mesh>
);
  ...
  return (
    <group ref={ref} position={[0, -0.3, 1.55]}>
      ...
      <mesh position={[0, -0.24, 0]}>
        ...
        <meshStandardMaterial color="#1f1c19" roughness={0.4} metalness={0.7} />
      </mesh>
      {/* Wheels */}
      {[0, 1, 2, 3, 4].map((i) => {
...
/** Desk lamp that rotates to point at a cursor world-position. */
interface LampProps {
  cursorWorld: THREE.Vector3;
  statusColor: string;
}
export const Lamp: React.FC<LampProps> = ({ cursorWorld, statusColor }) => {
  const headRef = useRef<THREE.Group>(null);
  const targetEuler = useMemo(() => new THREE.Euler(), []);
  const tmpDir = useMemo(() => new THREE.Vector3(), []);
  const baseWorld = useMemo(() => new THREE.Vector3(1.3, 0.16, -0.4), []);

  useFrame(() => {
    if (!headRef.current) return;
    tmpDir.copy(cursorWorld).sub(baseWorld);
    const len = tmpDir.length();
    if (len < 0.001) return;
    // Yaw to point at cursor (Y rotation)
    const yaw = Math.atan2(tmpDir.x, tmpDir.z);
    // Pitch (X rotation)
    const horiz = Math.sqrt(tmpDir.x * tmpDir.x + tmpDir.z * tmpDir.z);
    const pitch = -Math.atan2(tmpDir.y - 0.4, horiz);
    targetEuler.set(pitch, yaw, 0);
    headRef.current.rotation.x +=
      (targetEuler.x - headRef.current.rotation.x) * 0.08;
    headRef.current.rotation.y +=
      (targetEuler.y - headRef.current.rotation.y) * 0.08;
  });
  ...
  return (
    <group position={[1.3, 0.04, -0.4]}>
      ...
      <mesh castShadow>
        ...
        <meshStandardMaterial color="#1a1714" roughness={0.5} metalness={0.4} />
      </mesh>
      {/* Status LED ring on lamp base */}
      <mesh position={[0, 0.022, 0]} rotation={[-Math.PI / 2, 0, 0]}>
        <ringGeometry args={[0.04, 0.062, 24]} />
        <meshStandardMaterial
          color={statusColor}
      ...
      <group ref={headRef} position={[0, 0.4, 0]}>
        <mesh position={[0, 0.1, 0]} castShadow>
          <coneGeometry args={[0.1, 0.18, 18, 1, true]} />
          <meshStandardMaterial
            color="#13110f"
            roughness={0.4}
            metalness={0.6}
