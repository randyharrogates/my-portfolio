...
| 2.4 | Dome ceiling | `hub/dome.py` | Faceted geodesic half-dome from icosphere subdivision 2 (42 verts → 26 after top-half cull, 40 flat-shaded tris). Two named meshes per glb: `hub-dome-shell` (BackSide dark interior) + `hub-dome-lattice` (brass-emissive wireframe overlay via Wireframe modifier). Radius 4.5m, Y-scale 0.65 → 2.93m dome height above z=5.5m. | ✅ 2026-05-11 |
| 2.5 | Wall panels between columns | `hub/walls.py` | Six short parapet walls at hex-edge midpoints (column angles), 3m wide × 1m tall × 0.08m thick. Two named meshes: `hub-wall-slab` (dark teal base) + `hub-wall-frame` (5 brass bars: top/bottom/left/right + decorative mid-slat). Low parapet keeps alcove arches visible above. | ✅ 2026-05-11 |
| 2.6 | Skylight aperture mesh | `hub/skylight.py` | Hex donut (outer 0.72m / inner 0.52m, 0.06m thick, 24 verts / 24 faces) + hex emissive disc (0.48m radius) inside the donut. Mounted at `y = HALL_CEILING_HEIGHT + 0.05` so the existing volumetric god-ray cone in `Atmosphere.tsx` continues to spawn from this point. | ✅ 2026-05-11 |
| 2.7 | PolyHaven brass PBR material | `useBrassMaterial` hook in `Hub.tsx` | PolyHaven `metal_plate_02` (1k JPGs: diffuse + normal_gl + roughness). Maps live in `public/textures/hall/brass/`. Applied via `useTexture` + `MeshStandardMaterial`, tinted to brass via `color: #b8862a`, metalness 1.0, per-element repeat (column shaft 1×4, base/capital 2×1, lattice/frame 4×1, ring 1×1, inlay 3×3). | ✅ 2026-05-11 |
| 2.8 | PolyHaven dark-concrete floor material | `useFloorMaterial` hook in `Hub.tsx` | PolyHaven `concrete_floor_02` (1k JPGs: diffuse + normal_gl + roughness). Maps in `public/textures/hall/floor/`. Applied to `hub-floor-slab` only (inlay uses brass). Repeat 4×4 across the 6m flat-to-flat hex. metalness 0.1, roughness 0.85, tinted #3a4a4e. | ✅ 2026-05-11 |
